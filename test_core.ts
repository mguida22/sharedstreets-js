...
test("sharedstreets -- test osrm install", (t: any) => {
  ...
  } else t.comment("osrmBinPath not found");
...
test("sharedstreets -- intersection", (t: any) => {
  t.equal(
    sharedstreets.intersectionId(pt1),
    "afd3db07d9baa6deef7acfcaac240607",
    "intersectionId => pt1"
  );
  t.equal(
    sharedstreets.intersectionId(pt2),
    "f22b51a95400e250bff8d889a738c0b0",
    "intersectionId => pt2"
  );
  t.equal(
    sharedstreets.intersectionId(pt3),
    "eed5479e5315e5a2e71760cc70a4ac76",
    "intersectionId => pt3"
  );

  t.equal(
    sharedstreets.intersectionMessage(pt1),
    "Intersection 110.00000 45.00000",
    "intersectionMessage => pt1"
  );
  t.equal(
    sharedstreets.intersectionMessage(pt2),
    "Intersection -74.00339 40.63454",
    "intersectionMessage => pt2"
  );
  t.equal(
    sharedstreets.intersectionMessage(pt3),
    "Intersection -74.00411 40.63406",
    "intersectionMessage => pt3"
  );

  // Extras
  t.equal(
    sharedstreets.intersectionId([-74.00962750000001, 40.740100500000004]),
    "68ea64a9f5be2b3a219898387b3da3e8",
    "intersectionId => extra1"
  );
  t.equal(
    sharedstreets.intersectionMessage([-74.00962750000001, 40.740100500000004]),
    "Intersection -74.00963 40.74010",
    "intersectionMessage => extra1"
  );
  t.end();
});

test("sharedstreets -- referenceId", (t: any) => {
  const locationReferenceOutbound = sharedstreets.locationReference(
    [-74.0048213, 40.7416415],
    { outboundBearing: 208, distanceToNextRef: 9279 }
  );
  const locationReferenceInbound = sharedstreets.locationReference(
    [-74.0051265, 40.7408505],
    { inboundBearing: 188 }
  );
  const formOfWay = 2; // => "MultipleCarriageway"

  t.equal(
    locationReferenceOutbound.intersectionId,
    "6d9fe428bc29b591ca1830d44e73099d",
    "locationReferenceOutbound => intersectionId"
  );
  t.equal(
    locationReferenceInbound.intersectionId,
    "5a44762edbad541f0fb808c44c018105",
    "locationReferenceInbound => intersectionId"
  );

  var refHash = sharedstreets.generateHash(
    "Reference 2 -74.00482 40.74164 208 93 -74.00513 40.74085"
  );

  t.equal(
    sharedstreets.referenceMessage(
      [locationReferenceOutbound, locationReferenceInbound],
      formOfWay
    ),
    "Reference 2 -74.00482 40.74164 208 93 -74.00513 40.74085",
    "referenceId => pt1"
...
test("sharedstreets -- locationReference", (t: any) => {
  ...
  const locRef = sharedstreets.locationReference(
    [-74.0048213, 40.7416415],
    options
  );

  var intersectionHash = sharedstreets.generateHash(
    "Intersection -74.00482 40.74164"
  );
...
test("sharedstreets-pbf -- intersection", (t: any) => {
  var count = 1;
  for (var filepath of glob.sync(
    path.join("./", "test", "pbf", `*.intersection.6.pbf`)
  )) {
...
test("sharedstreets-pbf -- geometry", (t: any) => {
  var count = 1;
  for (var filepath of glob.sync(
    path.join("./", "test", "pbf", `*.geometry.6.pbf`)
  )) {
    const buffer = fs.readFileSync(filepath);
...
test("sharedstreets-pbf -- reference", (t: any) => {
  var count = 1;
  for (var filepath of glob.sync(
    path.join("./", "test", "pbf", `*.reference.6.pbf`)
  )) {
    const buffer = fs.readFileSync(filepath);
    ...
    for (var reference of references) {
      ...
      const expectedId = sharedstreets.referenceId(
        locationReferences,
        formOfWay
      );
      const message = sharedstreets.referenceMessage(
        locationReferences,
        formOfWay
      );
...
test("sharedstreets -- coordsToLonlats", (t: any) => {
  const lonlats = sharedstreets.coordsToLonlats([
    [110, 45],
    [120, 55],
  ]);
...
test("sharedstreets -- geometry", (t: any) => {
  ...
  var geomHash = sharedstreets.generateHash(
    "Geometry 110.00000 45.00000 115.00000 50.00000 120.00000 55.00000"
  );
...
test("sharedstreets -- reference", (t: any) => {
  ...
  const locationReferences = [
    sharedstreets.locationReference([-74.0048213, 40.7416415], {
      outboundBearing: 208,
      distanceToNextRef: 9279,
    }),
    sharedstreets.locationReference([-74.0051265, 40.7408505], {
      inboundBearing: 188,
    }),
  ];
  ...
  const refHash = sharedstreets.generateHash(
    "Reference 0 -74.00482 40.74164 208 93 -74.00513 40.74085"
  );
...
test("sharedstreets -- metadata", (t: any) => {
  ...
  const gisMetadata = [
    {
      source: "sharedstreets",
      sections: [{ sectionId: "foo", sectionProperties: "bar" }],
    },
  ];
  const geom = sharedstreets.geometry(line);
...
test("sharedstreets -- bearing & distance", (t: any) => {
  const line = lineString([
    [-74.006449, 40.739405000000005],
    [-74.00790070000001, 40.7393884],
    [-74.00805100000001, 40.7393804],
  ]);
  const lineLength = length(line);
  const inboundBearing = sharedstreets.inboundBearing(
    line,
    lineLength,
    lineLength
  );
...
test("cache -- load data", async (t: any) => {
  ...
  var tilePathGroup: TilePathGroup = TilePathGroup.fromPolygon(
    polygon,
    0,
    params
  );
...
test("tileIndex -- point data", async (t: any) => {
  ...
  var points: turfHelpers.FeatureCollection<turfHelpers.Point> = JSON.parse(
  ...
  for (var point of points.features) {
    var foundFeatures = await tileIndex.nearby(
      point,
      TileType.GEOMETRY,
      10,
      params
...
test("match lines 2 -- snapping and directed edges", async (t: any) => {
  ...
  for (var line of lines.features) {
    var pathCandidate = await matcher.matchGeom(line);
    if (pathCandidate) matchedLines.features.push(pathCandidate.matchedPath);
  }
