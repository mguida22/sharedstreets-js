...
import { TilePathParams, TileType } from "../index";
import {
  Graph,
  PathCandidate,
  GraphMode,
...
import {
  generateBinId,
  getBinCountFromLength,
  getBinPositionFromLocation,
  getBinLength,
} from "../data";
...
export default class Match extends Command {
  ...
  static flags = {
    ...
    "skip-port-properties": flags.boolean({
      char: "p",
      description:
        'skip porting existing feature properties preceeded by "pp_"',
      default: false,
    }),
  ...
  async run() {
    ...
    if (flags["direction-field"])
      console.log(
        chalk.bold.keyword("green")(
          '       Filtering one-way and two-way streets using field "' +
            flags["direction-field"] +
            '" with values: ' +
            ' "' +
            flags["one-way-with-direction-value"] +
            '", "' +
            flags["one-way-against-direction-value"] +
            '", "' +
            flags["two-way-value"] +
            '"'
        )
    ...
    if (flags["match-bike"] || flags["match-pedestrian"]) {
      if (flags["match-bike"]) {
        console.log(
          chalk.bold.keyword("green")(
            "       Matching using bike routing rules"
          )
        );
      }
      if (flags["match-pedestrian"]) {
        console.log(
          chalk.bold.keyword("green")(
            "       Matching using pedestrian routing rules"
          )
        );
      }
      if (flags["match-motorway-only"])
        console.log(
          chalk.bold.keyword("orange")("       Ignoring motorway-only setting")
        );
    } else if (flags["match-car"]) {
      if (flags["match-motorway-only"])
        console.log(
          chalk.bold.keyword("green")(
            "       Matching using car routing rules on motorways only"
          )
        );
      else if (flags["match-surface-only"])
        console.log(
          chalk.bold.keyword("green")(
            "       Matching using car routing rules on surface streets only"
          )
        );
      else
        console.log(
    ...
    var data: turfHelpers.FeatureCollection<turfHelpers.Geometry> = JSON.parse(
...
async function matchPoints(outFile, params, points, flags) {
  console.log(
    chalk.bold.keyword("green")(
      "  ✨  Matching " + points.features.length + " points..."
    )
  );
  ...
  const bar2 = new cliProgress.Bar(
    ...
    {
      format:
        chalk.keyword("blue")(" {bar}") + " {percentage}% | {value}/{total} ",
      barCompleteChar: "\u2588",
      barIncompleteChar: "\u2591",
    }
  );
  ...
  for (var searchPoint of cleanPoints.clean) {
    ...
    if (
      ...
      searchPoint.properties[flags["bearing-field"]]
    )
      bearing = parseFloat(searchPoint.properties[flags["bearing-field"]]);

    var matches: PointCandidate[] = await graph.matchPoint(
      searchPoint,
      bearing,
      3,
      flags["left-side-driving"]
    );
    if (matches.length > 0) {
  ...
  if (flags["cluster-points"]) {
    ...
    const mergePointIntoCluster = (matchedPoint: MatchedPointType) => {
      ...
      if (
        ...
        (matchedPoint.matchedPoint.location <=
          flags["snap-intersections-radius"] ||
          matchedPoint.matchedPoint.referenceLength -
            matchedPoint.matchedPoint.location <=
            flags["snap-intersections-radius"])
      ) {
        var reference = <SharedStreetsReference>(
          graph.tileIndex.objectIndex.get(matchedPoint.matchedPoint.referenceId)
        );
        var intersectionId;
        ...
        if (
          ...
          intersectionId =
            reference.locationReferences[
              reference.locationReferences.length - 1
            ].intersectionId;
        }
        ...
        if (intersectionClusteredPointMap[intersectionId]) {
          ...
          pointGeom = JSON.parse(
            JSON.stringify(graph.tileIndex.featureIndex.get(intersectionId))
          );
          var intersection = <SharedStreetsIntersection>(
            graph.tileIndex.objectIndex.get(intersectionId)
          );
          ...
          for (var inboundRefId of intersection.inboundReferenceIds) {
            pointGeom.properties[
              "inboundReferenceId_" + inboundCount
            ] = inboundRefId;
            inboundCount++;
          }
          ...
          for (var outboundRefId of intersection.outboundReferenceIds) {
            pointGeom.properties[
              "outboundReferenceId_" + outboundCount
            ] = outboundRefId;
            outboundCount++;
          }
        ...
        var binCount = getBinCountFromLength(
          matchedPoint.matchedPoint.referenceLength,
          flags["cluster-points"]
        );
        var binPosition = getBinPositionFromLocation(
          matchedPoint.matchedPoint.referenceLength,
          flags["cluster-points"],
          matchedPoint.matchedPoint.location
        );
        var binId = generateBinId(
          matchedPoint.matchedPoint.referenceId,
          binCount,
          binPosition
        );
        var binLength = getBinLength(
          matchedPoint.matchedPoint.referenceLength,
          flags["cluster-points"]
        );
        ...
        if (clusteredPointMap[binId]) {
          ...
          var bins = graph.tileIndex.referenceToBins(
            matchedPoint.matchedPoint.referenceId,
            binCount,
            2,
            ReferenceSideOfStreet.RIGHT
          );
          var binPoint = turfHelpers.point(
            bins.geometry.coordinates[binPosition - 0]
          );
          binPoint.properties["id"] = binId;
          binPoint.properties["referenceId"] =
            matchedPoint.matchedPoint.referenceId;
      ...
      for (var property of Object.keys(
        matchedPoint.originalFeature.properties
      )) {
        if (property.startsWith("pp_")) {
          if (!isNaN(matchedPoint.originalFeature.properties[property])) {
            ...
            pointGeom.properties[sumPropertyName] +=
              matchedPoint.originalFeature.properties[property];
          }
        }
      }
    };
    ...
    intersectionClusteredPoints = Object.keys(
      intersectionClusteredPointMap
    ).map((key) => intersectionClusteredPointMap[key]);
    clusteredPoints = Object.keys(clusteredPointMap).map(
      (key) => clusteredPointMap[key]
    );
  }
  ...
  if (flags["buffer-points"]) {
    ...
    console.log(
      chalk.bold.keyword("green")(
        "  ✨  Buffering " + matchedPoints.length + " matched points..."
      )
    );
    ...
    console.log(
      chalk.bold.keyword("green")(
        "        default buffer length: " + bufferLength
      )
    );

    var bufferLengthFieldName = null;
    if (flags["buffer-points-length-field"]) {
      bufferLengthFieldName = flags["buffer-points-length-field"]
        .toLocaleLowerCase()
        .trim()
        .replace(/ /g, "_");
      console.log(
        chalk.bold.keyword("green")(
          "        buffer length fieldname: " + bufferLengthFieldName
        )
      );
    }
    bufferLengthFieldName = flags["buffer-points-length-field"]
    ...
    for (var matchedPoint of matchedPoints) {
      ...
      if (offsetLine) {
        if (leftSideDriving) {
          if (
            matchedPoint.matchedPoint.sideOfStreet ===
            ReferenceSideOfStreet.LEFT
          ) {
      ...
      if (
        bufferLengthFieldName &&
        matchedPoint.originalFeature.properties.hasOwnProperty(
          bufferLengthFieldName
        )
      )
        pointBufferLength =
          matchedPoint.originalFeature.properties[bufferLengthFieldName];

      matchedPoint.bufferedPoint = await graph.bufferPoint(
    ...
    if (flags["buffer-merge"]) {
      const bufferIntersectionRaidus: number =
        flags["trim-intersections-radius"];

      console.log(
        chalk.bold.keyword("green")(
          "  ✨  Merging " + bufferedPoints.length + " buffered points..."
        )
      );

      let bufferedPreMergedPoints: Map<
        string,
        Array<MatchedPointType>
      > = new Map();
      ...
      if (flags["buffer-merge-match-fields"]) {
        ...
        console.log(
          chalk.bold.keyword("green")(
            "        merging on fields: " + mergeFields.join(", ")
          )
        );
      }
      ...
      if (flags["buffer-merge-group-fields"]) {
        ...
        console.log(
          chalk.bold.keyword("green")(
            "        grouping on field values: " + groupFields.join(", ")
          )
        );
      }
      ...
      for (let matchedPoint of matchedPoints) {
        ...
        for (let mergeField of mergeFields) {
          if (
            matchedPoint.originalFeature.properties.hasOwnProperty(mergeField)
          ) {
            fieldValues.push(
              (
                mergeField +
                ":" +
                matchedPoint.originalFeature.properties[mergeField]
              )
                .toLocaleLowerCase()
                .trim()
                .replace(/ /g, "_")
        ...
        let refSideHash =
          matchedPoint.bufferedPoint.referenceId +
          ":" +
          matchedPoint.bufferedPoint.sideOfStreet +
          ":" +
          fieldValuesString;
      ...
      let mergeSegments = async (
        bufferedSegments: MatchedPointType[]
      ): Promise<MergeBufferedPointsType[]> => {
        let mergedSegment = new MergeBufferedPointsType();
        let mergedSegments: MergeBufferedPointsType[] = [];

        bufferedSegments;

        bufferedSegments.sort(
          (a: MatchedPointType, b: MatchedPointType): number =>
            a.bufferedPoint.section[0] > b.bufferedPoint.section[0] ? 11 : -1
        );
        ...
        while (segment1 && bufferedSegments.length > 0) {
          ...
          if (
            segment2 &&
            mergedSegment.mergedPathSegments.isIntersecting(
              segment2.bufferedPoint
            )
          ) {
            ...
            if (offsetLine) {
              if (leftSideDriving) {
                if (
                  mergedSegment.mergedPathSegments.sideOfStreet ===
                  ReferenceSideOfStreet.LEFT
                ) {
                  offsetLine = offsetLine;
                } else if (
            ...
            mergedSegment.mergedPathSegments = await graph.union(
              mergedSegment.mergedPathSegments,
              segment2.bufferedPoint,
              bufferIntersectionRaidus,
              offsetLine
            );
        ...
        if (mergedSegment) mergedSegments.push(mergedSegment);
      ...
      for (let refSide of bufferedPreMergedPoints.keys()) {
        if (bufferedPreMergedPoints.get(refSide).length > 0) {
          let mergedBuffers: MergeBufferedPointsType[] = await mergeSegments(
            bufferedPreMergedPoints.get(refSide)
          );
          for (let mergedBuffer of mergedBuffers) {
            ...
            for (let mergeField of mergeFields) {
              if (
                mergedBuffer.matchedPoints[0].originalFeature.properties.hasOwnProperty(
                  mergeField
                )
              ) {
                outputBufferedFeature.properties["pp_" + mergeField] =
                  mergedBuffer.matchedPoints[0].originalFeature.properties[
                    mergeField
                  ];
              }
            }
            ...
            for (let groupField of groupFields) {
              ...
              for (let point of mergedBuffer.matchedPoints) {
                if (
                  point.originalFeature.properties.hasOwnProperty(groupField)
                ) {
                  groupedFieldValues.push(
                    point.originalFeature.properties[groupField]
                  );
                }
              }
              outputBufferedFeature.properties[
                "pp_" + groupField
              ] = groupedFieldValues;
            }

            outputBufferedFeature.properties["shst_merged_point_count"] =
              mergedBuffer.matchedPoints.length;
            let mergedBufferLength = 0;
            for (let point of mergedBuffer.matchedPoints) {
              mergedBufferLength +=
                point.bufferedPoint.section[1] - point.bufferedPoint.section[0];
            }
            outputBufferedFeature.properties[
              "shst_merged_buffer_length"
            ] = mergedBufferLength;
  ...
  if (flags["join-points"]) {
    ...
    console.log(
      chalk.bold.keyword("green")(
        "  ✨  Joining " + matchedPoints.length + " matched points..."
      )
    );
    ...
    const mergePoints = async (
      ...
      for (let matchedPoint of matchedPoints) {
        if (!currSegment) {
          ...
          if (
            parseInt(
              matchedPoint.originalFeature.properties[
                flags["join-point-sequence-field"]
              ]
            ) === 1
          ) {
            ...
            let startPoint: MatchedPointType = JSON.parse(
              JSON.stringify(matchedPoint)
            );
            ...
            if (
              ...
              currSegment.joinedPath = await graph.joinPoints(
                currSegment.matchedPoints[0].matchedPoint,
                currSegment.matchedPoints[currSegment.matchedPoints.length - 1]
                  .matchedPoint,
                bufferIntersectionRaidus,
                offsetLine
              );
      ...
      if (currSegment && currSegment.matchedPoints.length > 0) {
        let endPoint: MatchedPointType = JSON.parse(
          JSON.stringify(
            currSegment.matchedPoints[currSegment.matchedPoints.length - 1]
          )
        );
    ...
    for (let refSide of preMergedPoints.keys()) {
      if (preMergedPoints.get(refSide).length > 0) {
        let mergedPointSegments: JoinedPointsType[] = await mergePoints(
          preMergedPoints.get(refSide)
        );
        for (let mergedPointSegment of mergedPointSegments) {
          ...
          for (let mergeField of mergeFields) {
            if (
              mergedPointSegment.matchedPoints[0].originalFeature.properties.hasOwnProperty(
                mergeField
              )
            ) {
              outputJoinedFeature.properties["pp_" + mergeField] =
                mergedPointSegment.matchedPoints[0].originalFeature.properties[
                  mergeField
                ];
            }
          }

          outputJoinedFeature.properties["shst_joined_point_count"] =
            mergedPointSegment.matchedPoints.length;
  ...
  if (matchedPoints.length) {
    ...
    var matchedFeatureCollection: turfHelpers.FeatureCollection<turfHelpers.Point> = turfHelpers.featureCollection(
      featureArray
    );
  ...
  if (clusteredPoints.length) {
    ...
    var clusteredPointsFeatureCollection: turfHelpers.FeatureCollection<turfHelpers.Point> = turfHelpers.featureCollection(
      clusteredPoints
    );
  ...
  if (bufferedPoints.length) {
    console.log(
      chalk.bold.keyword("blue")(
        "  ✏️  Writing " +
          bufferedPoints.length +
          " buffered points: " +
          outFile +
          ".buffered.geojson"
      )
    );
    var bufferedPointsFeatureCollection: turfHelpers.FeatureCollection<turfHelpers.LineString> = turfHelpers.featureCollection(
      bufferedPoints
    );
  ...
  if (bufferedMergedPoints.length) {
    console.log(
      chalk.bold.keyword("blue")(
        "  ✏️  Writing " +
          bufferedMergedPoints.length +
          " buffered and merged points: " +
          outFile +
          ".buffered.merged.geojson"
      )
    );
    var bufferedMergedPointsFeatureCollection: turfHelpers.FeatureCollection<turfHelpers.LineString> = turfHelpers.featureCollection(
      bufferedMergedPoints
    );
    var bufferedMergedJsonOut = JSON.stringify(
      bufferedMergedPointsFeatureCollection
    );
  ...
  if (intersectionClusteredPoints.length) {
    console.log(
      chalk.bold.keyword("blue")(
        "  ✏️  Writing " +
          intersectionClusteredPoints.length +
          " intersection clustered points: " +
          outFile +
          ".intersection_clustered.geojson"
      )
    );
    var intersectionClusteredPointsFeatureCollection: turfHelpers.FeatureCollection<turfHelpers.Point> = turfHelpers.featureCollection(
      intersectionClusteredPoints
    );
    var intersectionClusteredJsonOut = JSON.stringify(
      intersectionClusteredPointsFeatureCollection
    );
    writeFileSync(
      outFile + ".intersection_clustered.geojson",
      intersectionClusteredJsonOut
    );
  }
  ...
  if (unmatchedPoints.length) {
    ...
    var unmatchedFeatureCollection: turfHelpers.FeatureCollection<turfHelpers.Point> = turfHelpers.featureCollection(
      unmatchedPoints
    );
  ...
  if (joinedPoints.length) {
    ...
    var joinedPointFeatureCollection: turfHelpers.FeatureCollection<turfHelpers.LineString> = turfHelpers.featureCollection(
      joinedPoints
    );
...
async function matchLines(outFile, params, lines, flags) {
  ...
  console.log(
    chalk.bold.keyword("green")(
      "  ✨  Matching " + cleanedlines.clean.length + " lines..."
    )
  );
  ...
  const getMatchedSegments = (
    ...
    for (var segment of path.segments) {
      ...
      segmentGeom.properties["shstFromIntersectionId"] =
        segment.fromIntersectionId;
      ...
      segmentGeom.properties["gisFromIntersectionId"] =
        ref.locationReferences[0].intersectionId;
      segmentGeom.properties["gisToIntersectionId"] =
        ref.locationReferences[
          ref.locationReferences.length - 1
        ].intersectionId;

      segmentGeom.properties["startSideOfStreet"] =
        path.startPoint.sideOfStreet;
      segmentGeom.properties["endSideOfStreet"] = path.endPoint.sideOfStreet;

      if (
        flags["side-of-street-field"] &&
        path.originalFeature.properties[flags["side-of-street-field"]]
      ) {
        var sideOfStreetValue = path.originalFeature.properties[
          flags["side-of-street-field"]
        ].toLocaleLowerCase();
        if (
          flags["left-side-of-street-value"].toLocaleLowerCase() ===
          sideOfStreetValue
        ) {
          path.sideOfStreet = ReferenceSideOfStreet.LEFT;
        } else if (
          flags["right-side-of-street-value"].toLocaleLowerCase() ===
          sideOfStreetValue
        ) {
          path.sideOfStreet = ReferenceSideOfStreet.RIGHT;
        } else if (
          flags["center-of-street-value"].toLocaleLowerCase() ===
          sideOfStreetValue
        ) {
        ...
        if (flags["snap-side-of-street"]) {
          if (flags["left-side-driving"]) {
            if (path.sideOfStreet == ReferenceSideOfStreet.RIGHT)
              segmentGeom = lineOffset(segmentGeom, 0 - flags["offset-line"], {
                units: "meters",
              });
            else if (path.sideOfStreet == ReferenceSideOfStreet.LEFT)
              segmentGeom = lineOffset(segmentGeom, flags["offset-line"], {
                units: "meters",
              });
          } else {
            if (path.sideOfStreet == ReferenceSideOfStreet.RIGHT)
  ...
  if (flags["match-bike"]) graphMode = GraphMode.BIKE;
  else if (flags["match-pedestrian"]) graphMode = GraphMode.PEDESTRIAN;
  else if (flags["match-car"]) {
    if (flags["match-motorway-only"]) graphMode = GraphMode.CAR_MOTORWAY_ONLY;
    else if (flags["match-surface-only"])
      graphMode = GraphMode.CAR_SURFACE_ONLY;
    else graphMode = GraphMode.CAR_ALL;
  } else graphMode = GraphMode.CAR_ALL;

  var matcher = new Graph(extent, params, graphMode);
  await matcher.buildGraph();

  if (flags["search-radius"]) matcher.searchRadius = flags["search-radius"];
  ...
  for (var line of cleanedlines.clean) {
    if (line.properties["geo_id"] == 30107269) console.log("30107269");

    var matchDirection: MatchDirection;
    if (
      flags["direction-field"] &&
      line.properties[flags["direction-field"].toLocaleLowerCase()] != undefined
    ) {
      var lineDirectionValue =
        "" + line.properties[flags["direction-field"].toLocaleLowerCase()];
    ...
    if (
      ...
      var reversedLine = <turfHelpers.Feature<turfHelpers.LineString>>(
        reverseLineString(line)
      );
        ...
        if (matchForward.score > matchBackward.score) {
          ...
          if (flags["left-side-driving"]) {
            if (matchForward.sideOfStreet == ReferenceSideOfStreet.LEFT)
              matchedLines = matchedLines.concat(matchForwardSegments);
            else matchedLines = matchedLines.concat(matchBackwardSegments);
          } else {
  ...
  if (matchedLines && matchedLines.length) {
    ...
    var matchedFeatureCollection: turfHelpers.FeatureCollection<turfHelpers.LineString> = turfHelpers.featureCollection(
      matchedLines
    );
    var matchedJsonOut = JSON.stringify(matchedFeatureCollection);
    writeFileSync(outFile + ".matched.geojson", matchedJsonOut);
  ...
  if (unmatchedLines && unmatchedLines.length) {
    ...
    var unmatchedFeatureCollection: turfHelpers.FeatureCollection<turfHelpers.LineString> = turfHelpers.featureCollection(
      unmatchedLines
    );
    var unmatchedJsonOut = JSON.stringify(unmatchedFeatureCollection);
    writeFileSync(outFile + ".unmatched.geojson", unmatchedJsonOut);
  ...
  if (cleanedlines.invalid && cleanedlines.invalid.length) {
    console.log(
      chalk.bold.keyword("blue")(
        "  ✏️  Writing " +
          cleanedlines.invalid +
          " in lines: " +
          outFile +
          ".invalid.geojson"
      )
    );
    var invalidFeatureCollection: turfHelpers.FeatureCollection<turfHelpers.LineString> = turfHelpers.featureCollection(
      cleanedlines.invalid
    );
