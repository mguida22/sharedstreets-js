...
export default class Extract extends Command {
  static description =
    "extracts SharedStreets streets using polygon boundary and returns GeoJSON output of all intersecting features";
  ...
  static flags = {
    ...
    metadata: flags.boolean({
      description: "Include SharedStreets OpenStreetMap metadata in output",
      default: false,
    }),
    tiles: flags.boolean({
      description: "Export list of tiles intersecting with bounding box",
      default: false,
    }),
  };
  ...
  async run() {
    ...
    this.log(
      chalk.bold.keyword("green")("  🗄️  Loading SharedStreets tiles...")
    );
    ...
    for (var feature of data.features) {
      ...
      if (flags.metadata) {
        feature.properties.metadata = tileIndex.metadataIndex.get(
