...
export class TileIndex {
  ...
  isIndexed(tilePath: TilePath): Boolean {
    if (this.tiles.has(tilePath.toPathString())) return true;
    else return false;
  }
  ...
  async indexTileByPath(tilePath: TilePath): Promise<boolean> {
    if (this.isIndexed(tilePath)) return true;

    var data = await getTile(tilePath);

    if (tilePath.tileType === TileType.GEOMETRY) {
      var geometryFeatures = [];
      for (var geometry of <SharedStreetsGeometry[]>data) {
      ...
      for (var intersection of <SharedStreetsIntersection[]>data) {
        if (!this.objectIndex.has(intersection.id)) {
          ...
          this.osmNodeIntersectionIndex.set(
            intersection.nodeId.toString(),
            intersection
          );
      ...
      for (var reference of <SharedStreetsReference[]>data) {
  ...
  ): Promise<turfHelpers.FeatureCollection<turfHelpers.Geometry>> {
    var tilePaths = TilePathGroup.fromPolygon(polygon, buffer, params);

    if (searchType === TileType.GEOMETRY) tilePaths.addType(TileType.GEOMETRY);
    else if (searchType === TileType.INTERSECTION)
      tilePaths.addType(TileType.INTERSECTION);
    else throw "invalid search type must be GEOMETRY or INTERSECTION";
  ...
  async nearby(
    point: turfHelpers.Feature<turfHelpers.Point>,
    searchType: TileType,
    searchRadius: number,
    params: TilePathParams
  ) {
    ...
    var bufferedPoint: turfHelpers.Feature<turfHelpers.Polygon> = buffer(
      point,
      searchRadius,
      { units: "meters" }
    );
  ...
  async geomFromOsm(
    ...
    nodeId2: string,
    offset: number = 0
  ): Promise<turfHelpers.Feature<turfHelpers.LineString | turfHelpers.Point>> {
    if (
      this.osmNodeIntersectionIndex.has(nodeId1) &&
      this.osmNodeIntersectionIndex.has(nodeId2)
    ) {
      var intersection1 = <SharedStreetsIntersection>(
        this.osmNodeIntersectionIndex.get(nodeId1)
      );
      var intersection2 = <SharedStreetsIntersection>(
        this.osmNodeIntersectionIndex.get(nodeId2)
      );
        ...
        for (var waySection of metadata.osmMetadata.waySections) {
          for (var nodeId of waySection.nodeIds) {
            ...
            if (previousNode != nodeIdStr) {
              ...
              if (nodeIdStr == nodeId1) startNodeIndex = nodeIndex;
              if (nodeIdStr == nodeId2) endNodeIndex = nodeIndex;
          ...
          if (startNodeIndex > endNodeIndex) {
            if (geometry.backReferenceId) {
              ...
              reference = <SharedStreetsReference>(
                this.objectIndex.get(geometry.backReferenceId)
              );
              geometryFeature = <turfHelpers.Feature<turfHelpers.LineString>>(
                JSON.parse(JSON.stringify(geometryFeature))
              );
            ...
            if (previousCoord) {
              try {
                var coordDistance = distance(
                  previousCoord,
                  geometryFeature.geometry.coordinates[j],
                  { units: "meters" }
                );
                if (j <= startNodeIndex) startLocation += coordDistance;
  ...
  referenceToBins(
    ...
    numBins: number,
    offset: number,
    _sideOfStreet: ReferenceSideOfStreet
  ): turfHelpers.Feature<turfHelpers.MultiPoint> {
    var binIndexId = referenceId + ":" + numBins + ":" + offset;

    if (this.binIndex.has(binIndexId)) return this.binIndex.get(binIndexId);
    ...
    var feature = <turfHelpers.Feature<turfHelpers.LineString>>(
      this.featureIndex.get(ref.geometryId)
    );
    ...
    try {
      ...
      for (var binPosition = 0; binPosition < numBins; binPosition++) {
        try {
          var point = along(feature, binLength * binPosition + binLength / 2, {
            units: "meters",
          });
          point.geometry.coordinates[0] =
            Math.round(point.geometry.coordinates[0] * 10000000) / 10000000;
          point.geometry.coordinates[1] =
            Math.round(point.geometry.coordinates[1] * 10000000) / 10000000;
  ...
  async geom(
    ...
    if (this.objectIndex.has(referenceId)) {
      var ref: SharedStreetsReference = <SharedStreetsReference>(
        this.objectIndex.get(referenceId)
      );
      var geom: SharedStreetsGeometry = <SharedStreetsGeometry>(
        this.objectIndex.get(ref.geometryId)
      );

      var geomFeature: turfHelpers.Feature<turfHelpers.LineString> = JSON.parse(
        JSON.stringify(this.featureIndex.get(ref.geometryId))
      );
