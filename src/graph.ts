...
function getReferenceLength(ref: SharedStreetsReference) {
  ...
  for (var lr of ref.locationReferences) {
    if (lr.distanceToNextRef) length += lr.distanceToNextRef;
  }
  return length / 100;
}
...
function roadClassConverter(roadClass: string): number {
  ...
  else if (roadClass === "Secondary") return 3;
  else if (roadClass === "Tertiary") return 4;
  else if (roadClass === "Residential") return 5;
  else if (roadClass === "Unclassified") return 6;
  else if (roadClass === "Service") return 7;
  else return null;
}
...
export class PathSegment {
  ...
  isIdentical(otherSegment: PathSegment): boolean {
    if (this.referenceId === otherSegment.referenceId) {
      if (
        otherSegment.section[0] === this.section[0] &&
        otherSegment.section[1] === this.section[1]
      )
        return true;
    }
    return false;
  ...
  isIntersecting(otherSegment: PathSegment): boolean {
    if (this.referenceId === otherSegment.referenceId) {
      if (
        (otherSegment.section[0] <= this.section[1] &&
          otherSegment.section[0] >= this.section[0]) ||
        (otherSegment.section[1] <= this.section[1] &&
          otherSegment.section[1] >= this.section[0])
      )
        return true;
    }
    return false;
  ...
  toFeature(): turfHelpers.Feature<turfHelpers.LineString> {
    var feature: turfHelpers.Feature<turfHelpers.LineString> = turfHelpers.feature(
...
export class PathCandidate {
  ...
  toDebugView(): turfHelpers.FeatureCollection<turfHelpers.Geometry> {
    var debugCollection = turfHelpers.featureCollection([
  ...
  getOriginalFeatureLength() {
    if (!this.originalFeatureLength)
      this.originalFeatureLength = length(this.originalFeature, {
        units: "meters",
      });
  ...
  getPathLength(): number {
    if (!this.pathLength) {
      ...
      for (var segment of this.segments) {
        if (segment.section)
          this.pathLength =
            this.pathLength + (segment.section[1] - segment.section[0]);
        else this.pathLength = this.pathLength + segment.referenceLength;
      }
    }
  ...
  isColinear(candidate: PathCandidate): boolean {
    if (
      ...
      this.segments.length == candidate.segments.length
    ) {
...
export class Graph {
  ...
  tileParams: TilePathParams | undefined;

  constructor(
    extent: turfHelpers.Feature<turfHelpers.Polygon>,
    tileParams: TilePathParams,
    graphMode: GraphMode = GraphMode.CAR_ALL,
  ...
  async createGraphXml() {
    ...
    var osmRootElem = xml.element({
      _attr: { version: "0.6", generator: "shst cli v1.0" },
    });
    ...
    const writeNode = async (
      lon: number,
      lat: number,
      shstIntersectionId: string = null
    ): Promise<number> => {
      var nodeId: number;
      ...
      if (shstIntersectionId && (await this.db.has(shstIntersectionId))) {
        ...
        // write node xml
        osmRootElem.push({
          node: [{ _attr: { id: nodeId, lat: lat, lon: lon } }],
        });
        nextNodeId++;
      }
    ...
    for (var obj of this.tileIndex.objectIndex.values()) {
      if (geomInstance(obj)) {
        if (obj.roadClass == "Motorway") {
          if (
            this.graphMode != GraphMode.CAR_ALL &&
            this.graphMode != GraphMode.CAR_MOTORWAY_ONLY
          ) {
            continue;
          }
        } else {
        ...
        if (obj.roadClass == "Other") {
          if (
            this.graphMode != GraphMode.BIKE &&
            this.graphMode != GraphMode.PEDESTRIAN
          ) {
            continue;
          }
        }
        ...
        for (var i = 0; i < coords.length; i++) {
          ...
          if (i === 0) shstIntersectionId = obj.fromIntersectionId;
          else if (i === coords.length - 1)
            shstIntersectionId = obj.toIntersectionId;

          var nodeId = await writeNode(
            coords[i][0],
            coords[i][1],
            shstIntersectionId
          );
        ...
        for (nodeId of nodeIds) {
          ...
          if (previousNode) {
            pushNodePair(
              "node-pair:" + nodeId + "-" + previousNode,
              obj.forwardReferenceId
            );
            if (obj.backReferenceId)
              pushNodePair(
                "node-pair:" + previousNode + "-" + nodeId,
                obj.backReferenceId
              );
          }
        ...
        if (roadClass == "other") roadClass = "path"; // TODO add bike/ped modal restrictions to paths

        osmRootElem.push({
          way: [
            { _attr: { id: nextEdgeId } },
            { tag: { _attr: { k: "highway", v: roadClass } } },
            { tag: { _attr: { k: "oneway", v: oneWay } } },
            ...nodeIdElems,
          ],
        });
  ...
  async buildGraph() {
    ...
    try {
      ...
      if (USE_LOCAL_CACHE && existsSync(dbPath)) {
        ...
        console.log(
          chalk.keyword("lightgreen")(
            "     building graph using OSRM from: " + OSRM_DIR
          )
        );
        ...
        console.log(
          chalk.keyword("lightgreen")(
            "     building " + this.graphMode + " graph  xml..."
          )
        );
        ...
        console.log(
          chalk.keyword("lightgreen")(
            "     building " + this.graphMode + " graph from: " + xmlPath
          )
        );
        ...
        if (
          this.graphMode === GraphMode.CAR_ALL ||
          this.graphMode === GraphMode.CAR_SURFACE_ONLY ||
          this.graphMode === GraphMode.CAR_MOTORWAY_ONLY
        )
        ...
        execSync(
          path.join(OSRM_DIR, "lib/binding/osrm-extract") +
            " " +
            xmlPath +
            " -p " +
            profile
        ...
        if (OPTIMIZE_GRAPH) {
          ...
          execSync(
            path.join(OSRM_DIR, "lib/binding/osrm-contract") + " " + osrmPath
          );
          // @ts-ignore
          this.osrm = new OSRM({ path: osrmPath });
        } else {
          execSync(
            path.join(OSRM_DIR, "lib/binding/osrm-partition") + " " + osrmPath
          );
          execSync(
            path.join(OSRM_DIR, "lib/binding/osrm-customize") + " " + osrmPath
          );
          console.log(
            chalk.keyword("lightgreen")("     skipping graph optimization...")
          );
          // @ts-ignore
          this.osrm = new OSRM({ path: osrmPath, algorithm: "MLD" });
        }
  ...
  async matchTrace(feature: turfHelpers.Feature<turfHelpers.LineString>) {
    ...
    var hmmOptions = {
      ...
      annotations: true,
      geometries: "geojson",
      radiuses: Array(feature.geometry.coordinates.length).fill(
        this.searchRadius
      ),
    };
    ...
    try {
        ...
        if (0 < match.confidence) {
          ...
          for (var leg of match["legs"]) {
            for (var n of leg["annotation"]["nodes"]) {
              if (!visitedNodes.has(n) || nodes.length == 0) nodes.push(n);
          ...
          for (var nodeId of nodes) {
            if (await this.db.has("node:" + nodeId)) {
              if (previousNode) {
                if (
                  await this.db.has("node-pair:" + nodeId + "-" + previousNode)
                ) {
                  ...
                  for (var edge of edges) {
                    if (!visitedEdges.has(edge)) visitedEdgeList.push(edge);
      ...
      for (var edgeReferenceId of visitedEdgeList) {
        var edgeRef = <SharedStreetsReference>(
          this.tileIndex.objectIndex.get(edgeReferenceId)
        );
        var edgeGeom = <SharedStreetsGeometry>(
          this.tileIndex.objectIndex.get(edgeRef.geometryId)
        );
        ...
        var segementGeom = <turfHelpers.Feature<turfHelpers.LineString>>(
          this.tileIndex.featureIndex.get(edgeGeom.id)
        );
        segmentGeoms.geometry.coordinates.push(
          segementGeom.geometry.coordinates
        );
      }
  ...
  async matchGeom(feature: turfHelpers.Feature<turfHelpers.LineString>) {
    ...
    try {
      ...
      if (visitedEdgeList.length > 0) {
        ...
        var endPoint = turfHelpers.point(
          feature.geometry.coordinates[feature.geometry.coordinates.length - 1]
        );

        var startCandidate: PointCandidate = await this.getPointCandidateFromRefId(
          startPoint,
          visitedEdgeList[0],
          null
        );
        var endCandidate: PointCandidate = await this.getPointCandidateFromRefId(
          endPoint,
          visitedEdgeList[visitedEdgeList.length - 1],
          null
        );
        ...
        for (var k = 0; k < visitedEdgeList.length; k++) {
          ...
          var shstRef = <SharedStreetsReference>(
            this.tileIndex.objectIndex.get(visitedEdgeList[k])
          );
          pathSegment.referenceId = visitedEdgeList[k];
          pathSegment.geometryId = shstRef.geometryId;
          pathCandidate.segments.push(pathSegment);
          pathCandidate.segments[k].referenceLength = getReferenceLength(
            shstRef
          );
        }
        ...
        if (pathCandidate.segments.length > 0) {
          ...
          for (var k = 0; k < pathCandidate.segments.length; k++) {
            var edgeGeom = <SharedStreetsGeometry>(
              this.tileIndex.objectIndex.get(
                pathCandidate.segments[k].geometryId
              )
            );

            pathCandidate.segments[k].roadClass = roadClassConverter(
              edgeGeom.roadClass
            );
            ...
            if (startCandidate.referenceId == endCandidate.referenceId) {
              ...
              pathCandidate.segments[k].section = [
              ...
              if (k == pathCandidate.segments.length - 1) {
                ...
                pathCandidate.segments[k].section = [
                  pathCandidate.startPoint.location,
                  pathCandidate.segments[k].referenceLength,
                ];
              } else {
                pathCandidate.segments[k].section = [
                  0,
            ...
            if (
              edgeGeom.forwardReferenceId ==
              pathCandidate.segments[k].referenceId
            ) {
              pathCandidate.segments[k].fromIntersectionId =
                edgeGeom.fromIntersectionId;
              pathCandidate.segments[k].toIntersectionId =
                edgeGeom.toIntersectionId;
            } else {
          ...
          if (
            pathCandidate.startPoint.sideOfStreet ==
            pathCandidate.endPoint.sideOfStreet
          )
            pathCandidate.sideOfStreet = pathCandidate.startPoint.sideOfStreet;
          else pathCandidate.sideOfStreet = ReferenceSideOfStreet.UNKNOWN;

          var startDist = distance(startPoint, startCandidate.pointOnLine, {
            units: "meters",
          });
          var endDist = distance(endPoint, endCandidate.pointOnLine, {
            units: "meters",
          });
          pathCandidate.score =
            Math.round(
              rmse([startDist, endDist, pathCandidate.getLengthDelta()]) * 100
            ) / 100;
    ...
    if (pathCandidates.length > 0) {
      ...
      for (var i = 0; i < bestPathCandidate.segments.length; i++) {
        totalPathLength +=
          bestPathCandidate.segments[i].section[1] -
          bestPathCandidate.segments[i].section[0];
      }
      ...
      for (var i = 0; i < bestPathCandidate.segments.length; i++) {
        ...
        if (
          segment.section[0] < segment.section[1] + this.searchRadius &&
          segment.section[1] <= segment.referenceLength + this.searchRadius &&
          segment.section[0] + this.searchRadius >= 0
        ) {
          if (this.snapIntersections && totalPathLength > this.searchRadius) {
            if (
              i == 0 &&
              segment.referenceLength - segment.section[0] < this.searchRadius
            )
              continue;
            ...
            if (
              i == bestPathCandidate.segments.length - 1 &&
              segment.section[1] < this.searchRadius
            )
              continue;

            if (
              i == bestPathCandidate.segments.length - 1 &&
              segment.referenceLength - segment.section[1] < this.searchRadius
            )
          ...
          if (segment.section[0] == segment.section[1]) continue;

          segment.geometry = await this.tileIndex.geom(
            segment.referenceId,
            segment.section[0],
            segment.section[1]
          );
      ...
      if (cleanedPath.length == 0) return null;
  ...
  async getPointCandidateFromRefId(
    ...
    var geometryFeature = <turfHelpers.Feature<LineString>>(
  ...
  getPointCandidateFromGeom(
    ...
    candidateGeomFeature: turfHelpers.Feature<LineString>,
    searchBearing: number,
    direction: ReferenceDirection
  ): PointCandidate {
    if (pointOnLine.properties.dist < this.searchRadius) {
  ...
  async joinPoints(
    ...
    if (intersectionBuffer * 2 < startPoint.referenceLength) {
      ...
      if (segmentStart > segmentEnd) {
        ...
        if (segmentStart < 0) segmentStart = intersectionBuffer;
      }
    }
    ...
    joinedPoint.geometry = <Feature<LineString>>(
      await this.tileIndex.geom(
        joinedPoint.referenceId,
        joinedPoint.section[0],
        joinedPoint.section[1],
        offsetLine
      )
    );
  ...
  async bufferPoint(
    ...
    bufferedPoint.geometry = <Feature<LineString>>(
      await this.tileIndex.geom(
        bufferedPoint.referenceId,
        bufferedPoint.section[0],
        bufferedPoint.section[1],
        offsetLine
      )
    );
  ...
  async union(
    ...
    otherSegment: PathSegment,
    bufferIntersectionRaidus: number,
    offsetLine: number
  ): Promise<PathSegment> {
    if (pathSegment.isIntersecting(otherSegment)) {
      pathSegment.section[0] = Math.min(
        pathSegment.section[0],
        otherSegment.section[0]
      );
      pathSegment.section[1] = Math.max(
        pathSegment.section[1],
        otherSegment.section[1]
      );
      ...
      if (bufferIntersectionRaidus * 2 < pathSegment.referenceLength) {
        ...
        if (
          pathSegment.referenceLength - pathSegment.section[1] <
          bufferIntersectionRaidus
        )
          pathSegment.section[1] =
            pathSegment.referenceLength - bufferIntersectionRaidus;
      }

      pathSegment.referenceLength =
        pathSegment.section[1] - pathSegment.section[0];
    }

    pathSegment.geometry = <Feature<LineString>>(
      await this.tileIndex.geom(
        pathSegment.referenceId,
        pathSegment.section[0],
        pathSegment.section[1],
        offsetLine
      )
  ...
  async matchPoint(
    ...
    var sortedCandidates = candidates.sort((p1, p2) => {
      ...
      if (p1.score === p2.score) {
        if (leftSideDrive) {
          if (
            p1.sideOfStreet === ReferenceSideOfStreet.LEFT &&
            p2.sideOfStreet === ReferenceSideOfStreet.RIGHT
          )
            return -1;
          else if (
            p2.sideOfStreet === ReferenceSideOfStreet.LEFT &&
            p1.sideOfStreet === ReferenceSideOfStreet.RIGHT
          )
            return 1;
        } else {
          if (
