...
class PathStackFrame {
  ...
  constructor(graph, start, startCost, visitedNodes, options) {
    ...
    this.edges = this.graph.incidentEdges(
      start,
      jkstra.OUT,
      this.options.edgeFilter
    );
  }
  ...
  findPath(start, end, endCoord: turfHelpers.Coord, min, max, options) {
    ...
    while (stack.length > 0) {
      ...
      while ((nextEdge = currentStackFrame.nextEdge())) {
        var remainingDistance = distance(nextEdge.data.start, endCoord, {
          units: "meters",
        });

        if (
          currentStackFrame.getTotalEdgeCost() -
            currentStackFrame.currentEdgeCost >
            0 &&
          currentStackFrame.getTotalEdgeCost() -
            currentStackFrame.currentEdgeCost +
            remainingDistance >
          ...
          if (
            currentStackFrame.getTotalEdgeCost() < max &&
            currentStackFrame.getTotalEdgeCost() > min
          ) {
          ...
          if (!currentStackFrame.isVisited(nextEdge.to)) {
            currentStackFrame = new PathStackFrame(
              ...
              nextEdge.to,
              currentStackFrame.getTotalEdgeCost(),
              currentStackFrame.visitedNodes,
              options
            );
