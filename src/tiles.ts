...
export function getTileIdsForPolygon(
  polygon: turfHelpers.Feature<turfHelpers.Polygon>,
  buffer: number = 0
): string[] {
  var polyBound = bbox(polygon);

  var nwPoint = destination([polyBound[0], polyBound[1]], buffer, 315, {
    units: "meters",
  });
  var sePoint = destination([polyBound[2], polyBound[3]], buffer, 135, {
    units: "meters",
  });
  let bounds = [
    nwPoint.geometry.coordinates[0],
    nwPoint.geometry.coordinates[1],
    sePoint.geometry.coordinates[0],
    sePoint.geometry.coordinates[1],
  ];

  return getTileIdsForBounds(bounds, false);
}

export function getTileIdsForPoint(
...
export function getTileIdsForBounds(
  bounds: number[],
  bufferEdge: boolean
): string[] {
  ...
  let bufferSize = 0;
  if (bufferEdge) bufferSize = 1;

  for (
    var x = tileRange.minX - bufferSize;
    x <= tileRange.maxX + bufferSize;
    x++
  ) {
    for (
      var y = tileRange.minY - bufferSize;
      y <= tileRange.maxY + bufferSize;
      y++
    ) {
...
export async function getTile(tilePath: TilePath): Promise<any[]> {
  ...
  if (USE_LOCAL_CACHE && existsSync(tileFilePath)) {
    ...
    if (USE_LOCAL_CACHE) {
      mkdirSync(path.join(SHST_TILE_CACHE_DIR, tilePath.source), {
        recursive: true,
      });
      writeFileSync(tileFilePath, arrayBuffer);
      console.log(
        chalk.keyword("lightgreen")("     writing to cache: " + tileFilePath)
      );
    }
  }
...
export class TilePathGroup extends TilePathParams {
  ...
  *[Symbol.iterator]() {
  ...
  addPath(path: TilePath) {
    ...
    else this.source = path.source;

    if (
      this.tileHierarchy != undefined &&
      this.tileHierarchy !== path.tileHierarchy
    )
      throw "Path source does not match group";
    else this.tileHierarchy = path.tileHierarchy;
