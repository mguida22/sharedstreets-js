...
export function envelopeBufferFromPoint(
  point,
  radius
...
export function reverseLineString(
  line: turfHelpers.Feature<turfHelpers.LineString>
): turfHelpers.Feature<turfHelpers.LineString> | turfHelpers.LineString {
  var reverseLineFeature: turfHelpers.Feature<turfHelpers.LineString> = JSON.parse(
    JSON.stringify(line)
  );
...
export class CleanedLines {
  ...
  constructor(inputData: any) {
    ...
    try {
      ...
      if (inputData.type === "FeatureCollection") {
        ...
        inputData.type === "LineString" ||
        inputData.type === "MultiLineString"
      ) {
        inputFeatures.push({
          type: "Feature",
          properties: {},
      ...
      for (var inputFeature of inputFeatures) {
        ...
        if (inputFeature.geometry.type === "LineString") {
          if (this.validLength(inputFeature)) this.clean.push(inputFeature);
          else this.invalid.push(inputFeature);
          ...
          if (inputFeature.geometry.coordinates.length == 1) {
            ...
            for (var lineStringCoordinates of inputFeature.geometry
              .coordinates) {
              if (newFeature.geometry.coordinates.length == 0) {
                newFeature.geometry.coordinates = lineStringCoordinates;
              } else if (
                newFeature.geometry.coordinates[
                  newFeature.geometry.coordinates.length - 1
                ][0] === lineStringCoordinates[0][0] &&
                newFeature.geometry.coordinates[
                  newFeature.geometry.coordinates.length - 1
                ][1] === lineStringCoordinates[0][1]
              ) {
                // continous line feature -- merge

                // remove duplicate end point
                newFeature.geometry.coordinates.splice(-1);
                newFeature.geometry.coordinates = newFeature.geometry.coordinates.concat(
                  lineStringCoordinates
                );
              } else {
                // disjoint line feature -- save current line and start over with new feature
                if (this.validLength(newFeature)) this.clean.push(newFeature);
                else this.invalid.push(newFeature);
  ...
  validLength(line: turfHelpers.Feature<turfHelpers.LineString>): boolean {
    if (line.geometry.coordinates.length > 1) return true;
    else return false;
  }
}
