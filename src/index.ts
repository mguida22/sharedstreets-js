 ...
 * @private
 */
export function geometryMessage(
  line: Feature<LineString> | LineString | number[][]
): string {
  const coords = getCoords(line);
  return (
    "Geometry " + coords.map(([x, y]) => `${round(x)} ${round(y)}`).join(" ")
  );
}
...
export function geometry(
  ...
  const toIntersectionId =
    forwardRef.locationReferences[forwardRef.locationReferences.length - 1]
      .intersectionId;
 ...
 * @private
 */
export function intersectionMessage(
  pt: number[] | Feature<Point> | Point,
  id?: string | number
): string {
...
export function intersection(
  ...
  const inboundReferenceIds = inboundReferences.map(
    (ref) => ref.intersectionId
  );
  const outboundReferenceIds = outboundReferences.map(
    (ref) => ref.intersectionId
  );
...
export function referenceMessage(
  ...
  locationReferences.forEach((lr) => {
    ...
    if (
      lr.outboundBearing !== null &&
      lr.outboundBearing !== undefined &&
      lr.distanceToNextRef !== null &&
      lr.distanceToNextRef !== undefined
    ) {
...
export function forwardReference(
  ...
  for (var i = 0; i < segmentCount + 1; i++) {
    ...
    if (i < segmentCount) {
      refProperties.outboundBearing = outboundBearing(
        line,
        lineLength,
        i * (lineLength / segmentCount)
      );
      refProperties.distanceToNextRef = Math.round(
        (lineLength / segmentCount) * 100
      );
    }
    if (i > 0) {
      refProperties.inboundBearing = inboundBearing(
        line,
        lineLength,
        i * (lineLength / segmentCount)
      );
    ...
    if (i == 0) lrCoord = getStartCoord(line);
    else if (i == segmentCount) lrCoord = getEndCoord(line);
    else {
...
export function locationReference(
  ...
  if (
    locRef.outboundBearing !== undefined &&
    locRef.distanceToNextRef === undefined
  ) {
    throw new Error(
      "distanceToNextRef is required if outboundBearing is present"
    );
  }
  return locRef;
}
 ...
 * outboundBearing; // => 208
 */
export function outboundBearing(
  line: Feature<LineString>,
  len: number,
  dist: number
): number {
 ...
 * inboundBearing; // => 188
 */
export function inboundBearing(
  line: Feature<LineString>,
  len: number,
  dist: number
): number {
